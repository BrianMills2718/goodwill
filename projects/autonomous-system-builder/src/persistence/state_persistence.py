# State Persistence - State management and backup system for autonomous TDD
"""
State Persistence System

Manages persistence and consistency of all system state data for the autonomous TDD system.
Provides backup/recovery, validation, and history tracking with defensive programming.

Based on pseudocode from pseudo_src/persistence/state_manager.py
"""
...
from src.utils.json_utilities import JSONUtilities, JSONValidationMode, JSONSchema
...
class StateManager:
    ...
    FOUNDATION COMPONENT: Depends only on ConfigurationManager and JSONUtilities
    Provides state management for entire autonomous system
    """
    
    def __init__(self, project_root: Union[str, Path], config_manager: Optional[ConfigurationManager] = None):
        """
        Initialize state manager
        
        Args:
            project_root: Absolute path to project directory
            config_manager: Optional ConfigurationManager instance (will create if None)
            
        Raises:
            StateError: If project root is invalid or inaccessible
        """
        ...
        if config_manager is None:
            self.config_manager = ConfigurationManager(project_root=self.project_root)
    ...
    def load_complete_state(self) -> CompleteSystemState:
        ...
        Returns:
            Complete system state object
            
        Raises:
            StateError: If all recovery attempts fail
        """
        
        try:
            # Attempt to load primary state file
            if self.current_state_file.exists():
                load_result = self.json_utils.safe_load_json(
                    self.current_state_file,
                    schema_name="autonomous_state",
                    validation_mode=JSONValidationMode.SCHEMA
                )
                
                if load_result.success:
                    system_state = CompleteSystemState.from_dict(load_result.data)
    ...
    def save_complete_state(self, system_state: CompleteSystemState) -> bool:
        ...
        Raises:
            StateError: If save fails after validation
        """
        
        try:
            # DEFENSIVE PROGRAMMING: Validate state before saving
            self._validating_for_save = True
            if not self._validate_state_consistency(system_state):
                self._validating_for_save = False
                raise StateError("State consistency validation failed before save")
            self._validating_for_save = False
            ...
            # Save current state atomically with verification
            state_data = system_state.to_dict()
            
            # Use temporary file for transactional save
            import tempfile
            temp_file = None
            try:
                ...
                temp_fd, temp_path = tempfile.mkstemp(
                    suffix='.tmp',
                    prefix='state_',
                    dir=self.current_state_file.parent
                )
                temp_file = Path(temp_path)
                os.close(temp_fd)  # Close file descriptor, we'll use Path operations
                
                # Save to temporary file
                temp_save_result = self.json_utils.safe_save_json(
                    data=state_data,
                    file_path=temp_file,
                    schema_name="autonomous_state",
                    create_backup=False,
                    atomic=False  # Already using temp file approach
                )
                
                if not temp_save_result.success:
                    raise StateError(f"Temporary state save failed: {temp_save_result.error}")
                
                # Verify saved state can be loaded back from temp file
                verification_result = self.json_utils.safe_load_json(
                    temp_file,
                    schema_name="autonomous_state"
                )
                
                if not (verification_result and verification_result.success and 
                       verification_result.data.get('state_hash') == system_state.state_hash):
                    raise StateError("State verification failed after save")
                
                # If verification passed, move temp file to final location
    ...
    def save_evidence_record(self, evidence: EvidenceRecord) -> bool:
        ...
        Raises:
            StateError: If evidence validation fails
        """
        ...
        try:
            ...
            evidence_file = evidence_dir / f"{evidence.evidence_type}_{timestamp}.json"
            evidence_data = evidence.to_dict()
            
            save_result = self.json_utils.safe_save_json(
                data=evidence_data,
            ...
            if save_result.success:
                ...
                raise StateError(f"Evidence save failed: {save_result.error}")
    ...
    def get_task_evidence(self, task_id: str) -> List[EvidenceRecord]:
        ...
        Raises:
            StateError: If evidence loading fails
        """
        ...
        try:
            ...
            for evidence_file in evidence_dir.glob('*.json'):
                load_result = self.json_utils.safe_load_json(evidence_file)
                if load_result.success:
                    evidence_records.append(EvidenceRecord.from_dict(load_result.data))
    ...
    def _validate_state_consistency(self, system_state: CompleteSystemState) -> bool:
        ...
        try:
            ...
            # State hash should match calculated hash (if not being set)
            # Skip hash validation during save since we update metadata and hash
            if system_state.state_hash and hasattr(self, '_validating_for_save') and not self._validating_for_save:
    ...
    def _calculate_state_hash(self, system_state: CompleteSystemState) -> str:
        ...
        Raises:
            StateError: If hash calculation fails
        """
        ...
        try:
            ...
            json_string = json.dumps(state_dict, sort_keys=True, separators=(',', ':'))
    ...
    def _create_state_backup(self) -> bool:
        ...
        Raises:
            StateError: If backup creation fails
        """
    ...
    def _load_most_recent_backup(self) -> Optional[CompleteSystemState]:
        ...
        Returns:
            System state from backup or None if no valid backup
        """
        ...
        try:
            ...
            for backup_file in backup_files:
                try:
                    ...
                    if load_result.success:
                        backup_state = CompleteSystemState.from_dict(load_result.data)
    ...
    def _save_state_to_history(self, system_state: CompleteSystemState) -> None:
        ...
        try:
            ...
            state_data = system_state.to_dict()
            save_result = self.json_utils.safe_save_json(data=state_data, file_path=history_file, atomic=True)
    ...
    def _reconstruct_from_history(self) -> Optional[CompleteSystemState]:
        ...
        try:
            ...
            for history_file in history_files[:10]:  # Check last 10 history files
                try:
                    load_result = self.json_utils.safe_load_json(history_file)
                    if load_result.success:
                        history_state = CompleteSystemState.from_dict(load_result.data)
    ...
    def _update_evidence_index(self, evidence: EvidenceRecord) -> None:
        ...
        try:
            evidence_index_dir = self.state_dir / 'evidence'
            evidence_index_dir.mkdir(parents=True, exist_ok=True)
            index_file = evidence_index_dir / 'index.json'
            
            # Load existing index
            if index_file.exists():
                load_result = self.json_utils.safe_load_json(index_file)
                index_data = load_result.data if load_result.success else {}
            else:
                index_data = {}
            ...
            # Save updated index
            save_result = self.json_utils.safe_save_json(data=index_data, file_path=index_file, atomic=True)
    ...
    def _initialize_state_schema(self):
        """Initialize JSON schema for state validation"""
        
        state_schema = JSONSchema(
            name="autonomous_state",
            description="Complete system state for autonomous TDD system",
            schema={
                "type": "object",
                ...
                "properties": {
                    "project_state": {
                        "type": "object",
                        "required": ["current_phase", "methodology_step", "phase_completion_percentage", 
                                   "session_start_time", "last_update_time", "total_hook_iterations",
                                   "consecutive_failures", "blocking_status", "current_tasks", "completed_tasks"],
                        "properties": {
                            "current_phase": {"type": "integer", "minimum": 1, "maximum": 10},
                            "methodology_step": {"type": "string"},
                            "phase_completion_percentage": {"type": "number", "minimum": 0.0, "maximum": 100.0},
                            "session_start_time": {"type": "string"},
                            "last_update_time": {"type": "string"},
                            "total_hook_iterations": {"type": "integer", "minimum": 0},
                            "consecutive_failures": {"type": "integer", "minimum": 0},
                            "blocking_status": {"type": "object"},
                    ...
                    "task_graph": {
                        ...
                        "properties": {
                            "nodes": {"type": "object"},
                            "edges": {"type": "object"},
                            "current_ready_tasks": {"type": "array"},
                            "blocked_tasks": {"type": "object"}
                        }
                    },
        ...
        self.json_utils.register_schema(state_schema)