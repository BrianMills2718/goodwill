# Configuration Manager - Configuration management with validation and environment overrides
"""
...
Based on pseudocode from pseudo_src/utils/configuration_manager.py
"""
...
class ConfigurationManager:
    ...
    def __init__(self, config_dir: Optional[Path] = None, project_root: Optional[Path] = None):
        ...
        try:
            if not reload and self.config_values and self.last_reload_time:
                ...
                return ConfigLoadResult(
                    ...
                    error="Configuration already loaded (use reload=True to force reload)"
                )
            ...
            sources_loaded = [result.source for result in load_results if result.success and result.source]
            errors = [error for result in load_results for error in result.errors if result.errors]
    ...
    def _load_env_config_file(self) -> ConfigLoadResult:
        """Load environment-specific configuration file"""
        return self._load_config_file(self.env_config_file, ConfigSource.CONFIG_FILE)
    ...
    def _get_nested_value(self, key: str) -> Optional[ConfigValue]:
        """Get configuration value supporting nested keys"""
        return self.config_values.get(key)
    
    def _get_type_hint_for_key(self, key: str) -> Type:
        """Get type hint for configuration key from schema"""
        # Extract section and key from dotted notation
        parts = key.split('.')
        if len(parts) >= 2:
            section = parts[0]
            config_key = parts[1]
            if section in self.config_schemas:
                return self.config_schemas[section].type_hints.get(config_key, str)
        return str
    
    def _get_description_for_key(self, key: str) -> str:
        """Get description for configuration key from schema"""
        # Extract section and key from dotted notation
        parts = key.split('.')
        if len(parts) >= 2:
            section = parts[0]
            config_key = parts[1]
            if section in self.config_schemas:
                return self.config_schemas[section].descriptions.get(config_key, "No description available")
    ...
    def _validate_single_value(self, key: str, value: Any) -> ConfigValidationResult:
        """Validate single configuration value"""
        # Simple validation - in full implementation would check against schema rules
        return ConfigValidationResult(valid=True)
    
    def _validate_complete_configuration(self) -> ConfigValidationResult:
        ...
        for section_name, schema in self.config_schemas.items():
            for required_key in schema.required_keys:
                full_key = f"{section_name}.{required_key}"
                if full_key not in self.config_values:
                    errors.append(f"Required configuration key missing: {full_key}")
    ...
    def _log_config_error(self, message: str):
        ...
        # In full implementation, would use proper logging
